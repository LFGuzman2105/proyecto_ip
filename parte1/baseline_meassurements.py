...
print("="*70)
print("ANÁLISIS DE VELOCIDAD DE LECTURA - ESTADÍSTICAS BÁSICAS")
print("="*70)
...
# Calcular tendencia (primera semana vs segunda semana)
mitad = len(evolucion) // 2
primera_semana_luis = evolucion['Luis'].iloc[:mitad].mean()
segunda_semana_luis = evolucion['Luis'].iloc[mitad:].mean()
primera_semana_byron = evolucion['Byron'].iloc[:mitad].mean()
segunda_semana_byron = evolucion['Byron'].iloc[mitad:].mean()

print(f"\nTendencia (Primera vs Segunda Semana):")
print(f"  Luis:  {primera_semana_luis:.1f} → {segunda_semana_luis:.1f} ppm ({segunda_semana_luis - primera_semana_luis:+.1f})")
print(f"  Byron: {primera_semana_byron:.1f} → {segunda_semana_byron:.1f} ppm ({segunda_semana_byron - primera_semana_byron:+.1f})")
...
Tendencia general:
  • Ambos lectores {'mejoraron' if segunda_semana_luis > primera_semana_luis else 'mantuvieron'} su velocidad en la segunda semana
""")
...
fig.suptitle('Gráficos de Control de Shewhart - Baseline', fontsize=16, fontweight='bold')
